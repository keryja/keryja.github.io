...
function leftPad(str, width, fillChar) {
  return (str + '').padStart(width, fillChar);
}
...
function localiseTimes(timezoneOffset) {
  // Set timezone display name (abbreviation)
  const timezoneAbbr = moment.tz(new Date(), moment.tz.guess()).zoneAbbr();
  document.querySelectorAll('.timezone').forEach(elem => {
    elem.innerHTML = timezoneAbbr;
  });

  // Update hour displays to match timezone
  document.querySelectorAll('.schedule span').forEach(elem => {
    const [start, end] = elem.innerHTML.split(' - ');
    const startMin = timeStrToMins(start) + timezoneOffset;
    const endMin = timeStrToMins(end) + timezoneOffset;

    elem.innerHTML = minsToTimeStr(startMin) + ' - ' + minsToTimeStr(endMin);
  });
}

function main(evt) {
  const timezoneOffset = moment().utcOffset() - moment.tz('CET').utcOffset();
  localiseTimes(timezoneOffset);
}
